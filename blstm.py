...
class BRLSTM(nn.Module):
    def __init__(self, i, h, o1, o2):
        super().__init__()
        self._lstm_cell = nn.LSTMCell(i, h)
        self._fc1t = nn.Linear(h, TEMP_HIDDEN)
        self._fc2t = nn.Linear(h, TEMP_HIDDEN)
        self._fc1 = nn.Linear(TEMP_HIDDEN, o1)  # 训练故障类型的全连接层，比如滚珠、内外圈故障
        self._fc2 = nn.Linear(TEMP_HIDDEN, o2)  # 训练故障大小的全连接层，比如0，0.07等
        self._hidden_size = h

    def forward(self, x):
        h_t = Variable(torch.zeros((x.size(0), self._hidden_size)).cuda())
        c_t = Variable(torch.zeros((x.size(0), self._hidden_size)).cuda())
        seq_len = x.shape[2]
        for i in range(seq_len):
            h_t, c_t = self._lstm_cell(x[:, :, i], (h_t, c_t))

        fc0 = nn.functional.relu(h_t)
...
# 准备训练模型
    model = BRLSTM(14, 5, 4, 4).cuda()
    torch.backends.cudnn.benchmark = True
    optimizer1 = optim.Adam(model.parameters(), lr=0.001, weight_decay=0.005)
